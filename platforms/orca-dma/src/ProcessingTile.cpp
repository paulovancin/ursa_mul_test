...
ProcessingTile::ProcessingTile(uint32_t x, uint32_t y) : Tile(x, y) {
	...
	_cpu->InitCounters(
		...
		CPU_COUNTER_MUL_ADDR, CPU_COUNTER_CYCLES_TOTAL_ADDR, CPU_COUNTER_CYCLES_STALL_ADDR,
	...
	_cpu->GetSignalCounterMul()->MapTo(
			(uint32_t*)(_mem0->GetMap(CPU_COUNTER_MUL_ADDR)), CPU_COUNTER_MUL_ADDR);
